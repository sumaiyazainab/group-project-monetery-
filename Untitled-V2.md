...
```python
# assumptions:
# - WT sequence is correct
# - all true WT samples are identical to it
# - variants differ by mutations, not truncations
# - no recombination or rearrangements
```


```python
def compute_activity_scores_with_wt_sequence(
    df,
    dna_col,
    protein_col,
    generation_col,
    ...
    # Load WT reference
    wt_seq = str(SeqIO.read(wt_fasta_path, "fasta").seq)

    # Detect WT rows by sequence
    def is_wt(seq):
        aln = pairwise2.align.globalxx(wt_seq, seq, one_alignment_only=True)[0]
        return aln.seqA == aln.seqB

    df["is_wt_sequence"] = df[sequence_col].apply(is_wt)
    ...
    if wt.empty:
        raise ValueError("No WT sequences detected using reference FASTA.")

    baseline = wt.groupby(generation_col)["beta_hat"].mean()

    df["wt_baseline_generation"] = df[generation_col].map(baseline)
