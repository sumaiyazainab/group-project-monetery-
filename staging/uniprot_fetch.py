...
def extract_uniprot_aa_sequence(uniprot_record: dict) -> str:
    ...
    try:
        protein_seq = uniprot_record["sequence"]["value"]
        return protein_seq
...
def extract_key_domains(features_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter Domain-only features completes the "key domains" requirement in the briefing
    key_domains_df = features_df[features_df["type"] == "Domain"]
